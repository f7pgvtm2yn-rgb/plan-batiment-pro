...
function mesh(data){const triangles=[],lines=[],issues=[];let maxDepth=30;
 ...
 for(const e of data.elements){if(walls.includes(e)||e.type==='slopedBeam')continue;const l=data.ls.get(e.levelId);if(!l)continue;const z=Number.isFinite(e.zBase)?e.zBase:l.elevation,h=Number(e.height);let ps=e.polygon;
