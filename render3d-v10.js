/* v0.14.2 depth-buffered solid rendering.
...
function mesh(data){const triangles=[],lines=[],issues=[];let maxDepth=30;
 ...
 function wallWedge(e,color){if(!e.a||!e.b||![e.a.x,e.a.y,e.b.x,e.b.y,e.bottomA,e.bottomB,e.topA,e.topB,e.thickness].every(Number.isFinite))return;const dx=e.b.x-e.a.x,dy=e.b.y-e.a.y,L=Math.hypot(dx,dy);if(L<1e-6)return;const nx=-dy/L*e.thickness/2,ny=dx/L*e.thickness/2,col=rgb(color),front=col.map(x=>Math.min(1,x*.88)),side=col.map(x=>Math.min(1,x*.74)),A=[e.a.x+nx,e.a.y+ny,e.bottomA],B=[e.b.x+nx,e.b.y+ny,e.bottomB],C=[e.b.x+nx,e.b.y+ny,e.topB],D=[e.a.x+nx,e.a.y+ny,e.topA],A2=[e.a.x-nx,e.a.y-ny,e.bottomA],B2=[e.b.x-nx,e.b.y-ny,e.bottomB],C2=[e.b.x-nx,e.b.y-ny,e.topB],D2=[e.a.x-nx,e.a.y-ny,e.topA];const quad=(a,b,c,d,co)=>{for(const p of [a,b,c,a,c,d])put(triangles,p,co);};quad(A,B,C,D,front);quad(A2,D2,C2,B2,front);quad(D,C,C2,D2,col);quad(A,A2,B2,B,side);quad(A,D,D2,A2,side);quad(B,B2,C2,C,side);}
 ...
 for(const e of data.elements){if(walls.includes(e)||e.type==='slopedBeam'||e.type==='roofSurface'||e.type==='roofWallExtension')continue;const l=data.ls.get(e.levelId);if(!l)continue;const z=Number.isFinite(e.zBase)?e.zBase:l.elevation,h=Number(e.height);let ps=e.polygon;
